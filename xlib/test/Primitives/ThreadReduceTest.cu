...
enum ThreadReduceOP { SUM_OP, MIN_OP, MAX_OP, LOGIC_AND_OP };

__device__ int devTest = 0;

__global__ void threadReduceTest2() {
    int value1 = 3;
    int value2 = 2;
    WarpReduce<>::AtomicAdd(value1, &devTest,
                            value2, &devTest);
    printf("%d \t %d \t %d\n", threadIdx.x, value1, value2);
}
