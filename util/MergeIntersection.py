...
def countIntersection(u, v, threads_per_intersect):
	...
	# account here in this step is off
	for diag_id in diag_ids[1:]:
		if diag_id < len(u):
			...
			ui_start, ui_end = diag_id-1, 0
			vi_start, vi_end = diag_id - len(u), diag_id
		else:
			ui_start, ui_end = len(u), diag_id - len(v)
			vi_start, vi_end = diag_id - len(v), len(v)
		print("bSearch: ({}, {}) -> ({}, {})".format(vi_start, ui_start, vi_end, ui_end))
	...
	for i in range(len(pathPoints)-1): 
		...
		triangle_count += intersectCount(u, v, pt_curr[0], pt_curr[1], pt_next[0] - vi_offset, pt_next[1] - ui_offset, work)
